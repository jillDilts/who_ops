...
df_traverse <- function(node, visit_function, parents = NULL) {
  
  # visit this node:
  visit_function(node, parents)
  
  # for each child node:
  for (child_node in node$childOrgs) {
    
    #recursively traverse children
    df_traverse(node = get_org(child_node$orgId),
                visit_function = visit_function,
                parents = c(parents, node$orgName))
  }
}

df_visit <- function(node, parents) {
  print(paste0(node$orgName, ": parents = ", parents))
}

df_traverse(get_org(4827), df_visit) 
```
